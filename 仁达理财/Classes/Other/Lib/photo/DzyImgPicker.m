...
//父类的view
@property (nonatomic, strong ) UIViewController *parentV;
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    switch (buttonIndex) {
        ...
        case 1:
        {
            NSLog(@"相册");
            if (self.maxNum-imageArray.count == 0) {
                UIAlertView *al = [[UIAlertView alloc] initWithTitle:@"提示" message:@"选取图片最大为6张!" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"知道了", nil];
...
#warning mark 更改添加按钮背景图片 
        cell.imgView.image = [UIImage imageNamed:@"addimg"];
...
#warning mark 修改背景颜色
    cell.imgView.backgroundColor = [UIColor  colorWithPatternImage:[UIImage imageNamed:@"addimg"]];
    cell.imgView.contentMode = UIViewContentModeScaleToFill;
    return cell;
}
